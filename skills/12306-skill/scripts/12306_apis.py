...
def _parse_start_datetime(train_date: str, start_time: str) -> datetime:
    """解析发车时间，兼容 12306 用 24:xx 表示跨日零点的情况"""
    start_hours, start_minutes = [int(x) for x in start_time.split(":")]
    start_date = _parse_start_train_date(train_date)

    if start_hours >= 24:
        start_date += timedelta(days=1)
        start_hours -= 24

    return start_date.replace(hour=start_hours, minute=start_minutes)
...
def parse_tickets_info(tickets_data: list[dict[str, Any]], map: dict[str, str]) -> list[dict[str, Any]]:
    ...
    for ticket in tickets_data:
        ...
        start_date = _parse_start_datetime(ticket["start_train_date"], ticket["start_time"])
...
def _cache_dir() -> str:
    """缓存目录：优先取 Rust 传入的可写目录（BONGO_CACHE_DIR），回退到脚本所在目录。"""
    return os.environ.get("BONGO_CACHE_DIR") or os.path.dirname(os.path.abspath(__file__))


def init() -> None:
    global STATIONS, CITY_STATIONS, CITY_CODES, NAME_STATIONS, LCQUERY_PATH
    lcquery_path_cache_file = os.path.join(_cache_dir(), "lcquery_path")
    if os.path.exists(lcquery_path_cache_file) and os.path.getmtime(lcquery_path_cache_file) > time.time() - 86400: # 缓存一天
        ...
        try:
            ...
            # lcQuery 接口可能改版导致中转查询路径获取失败；降级为空，不影响直达查询
            LCQUERY_PATH = ""
    
    stations_cache_file = os.path.join(_cache_dir(), "stations.json")
